...
export default function CreateQuestionScreen() {
  ...
  const handleSave = async () => {
    ...
    if (!correctAnswer) {
      Alert.alert("Lupa!", "Pilih kunci jawaban dulu (klik tombol bulat).");
      return;
    }
    const options = [optA, optB, optC, optD];
    if (!options.includes(correctAnswer)) {
      Alert.alert("Error", "Teks opsi berubah. Klik ulang kunci jawaban.");
      return;
    }

    setSaving(true);
    try {
      await addQuestion(categoryId as string, questionText, options, correctAnswer, difficulty);
  ...
  return (
    // PERBAIKAN DI SINI: Menambahkan keyboardVerticalOffset
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Tambah Soal Baru</Text>
          <View style={{width: 24}} /> 
        </View>
        ...
        >
          ...
          />
          <Text style={styles.instruction}>Isi opsi, lalu <Text style={{fontWeight:'bold', color:'#2196F3'}}>KLIK TOMBOL BULAT</Text> untuk kunci jawaban.</Text>
            ...
            {[
              ...
              <View key={idx} style={styles.row}>
                ...
                <TextInput 
                  ...
                  placeholder={opt.ph} 
                  value={opt.val} 
                  onChangeText={(t) => { opt.set(t); if(isSelected(opt.val)) setCorrectAnswer(''); }}
                />
              </View>
            ))}
          </View>
          ...
          <TouchableOpacity style={styles.btnSave} onPress={handleSave} disabled={saving}>
            <Text style={styles.btnText}>{saving ? "Menyimpan..." : "Simpan Soal"}</Text>
          </TouchableOpacity>
        </ScrollView>
      </View>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', justifyContent: 'space-between', padding: 20, paddingTop: 40, backgroundColor: 'white', elevation: 2, alignItems: 'center' },
  title: { fontSize: 18, fontWeight: 'bold' },
  label: { fontSize: 14, fontWeight: 'bold', color: '#555', marginBottom: 8 },
  instruction: { fontSize: 12, color: '#666', marginBottom: 10, marginTop: 10 },
  input: { backgroundColor: 'white', borderWidth: 1, borderColor: '#ddd', borderRadius: 10, padding: 15, fontSize: 13 },
  ...
  inputFlex: { flex: 1, backgroundColor: 'white', borderWidth: 1, borderColor: '#ddd', borderRadius: 10, padding: 12 },
  btnSave: { backgroundColor: '#2196F3', padding: 15, borderRadius: 10, alignItems: 'center', marginTop: 30 },
